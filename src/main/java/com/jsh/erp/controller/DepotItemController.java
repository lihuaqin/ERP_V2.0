...
import com.jsh.erp.datasource.vo.SaleOrderExcelVO;
...
public class DepotItemController {
    ...
    public BaseResponseInfo getDetailList(@RequestParam("headerId") Long headerId,
                              @RequestParam("mpList") String mpList, HttpSession session,
                              HttpServletRequest request)throws Exception {
        BaseResponseInfo res = new BaseResponseInfo();
        Map<String, Object> map = new HashMap<String, Object>();
        Long tenantId = Long.parseLong(request.getSession().getAttribute("tenantId").toString());
        try {
            ...
            session.setAttribute("headerId", headerId);
    ...
    public void exportDetailList(
        ...
        Map<String, Object> map = new HashMap<String, Object>();
        Long tenantId = Long.parseLong(request.getSession().getAttribute("tenantId").toString());
        Long headerId = Long.parseLong(request.getSession().getAttribute("headerId").toString());
        String mpList = request.getSession().getAttribute("mpList").toString();
        try {
            List<DepotItemVo4WithInfoEx> dataList = new ArrayList<DepotItemVo4WithInfoEx>();
            if(headerId != 0) {
                dataList = depotItemService.getDetailList(headerId);
            ...
            outer.put("total", dataList.size());
            //存放数据json数组
            List<SaleOrderExcelVO> exportList = new ArrayList<SaleOrderExcelVO>();
            BigDecimal toalPrice = new BigDecimal("0");
            if (null != dataList) {
                for (DepotItemVo4WithInfoEx diEx : dataList) {
                    ...
                    saleOrderExcelVO.setDepotName(diEx.getDepotid() == null ? "" : diEx.getDepotName());
                    saleOrderExcelVO.setMaterialName(MaterialName);
                    saleOrderExcelVO.setStock( depotItemService.getStockByParam(diEx.getDepotid(),diEx.getMaterialid(),null,null,tenantId));
                    saleOrderExcelVO.setUnit(diEx.getMunit());
                    saleOrderExcelVO.setOperNumber( diEx.getOpernumber());
                    saleOrderExcelVO.setFinishNumber(depotItemService.getFinishNumber(diEx.getMaterialid(),diEx.getHeaderid()));
                    saleOrderExcelVO.setUnitPrice(diEx.getUnitprice());
                    saleOrderExcelVO.setAllPrice(diEx.getAllprice());
                    saleOrderExcelVO.setRemark(diEx.getRemark());
                    exportList.add(saleOrderExcelVO);
                    toalPrice = toalPrice.add(diEx.getAllprice());
            ...
            response.setHeader("Content-disposition", "attachment;filename=sale_order.xls");

            //刷新缓冲
            response.flushBuffer();

            //workbook将Excel写入到response的输出流中，供页面下载
            depotItemService.exoprtSaleOrderExcel(exportList , toalPrice ,response.getOutputStream());
